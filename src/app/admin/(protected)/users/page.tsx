...
import { StatusInlineControl } from '@/components/admin/StatusInlineControl';
...
export default function AdminUsersPage() {
	...
	const [verifiedF, setVerifiedF] = useState<'any' | 'true' | 'false'>('any');
	const [emailVerifiedF, setEmailVerifiedF] = useState<'any' | 'true' | 'false'>('any');
	...
	const params = useMemo(() => {
		...
		if (verifiedF !== 'any') sp.set('verifiedPublic', verifiedF);
		if (emailVerifiedF !== 'any') sp.set('emailVerified', emailVerifiedF);
		sp.set('page', String(page));
		sp.set('pageSize', String(pageSize));
		return sp.toString();
	}, [q, role, status, verifiedF, emailVerifiedF, page]);

	useEffect(() => {
		window.scrollTo(0, 0);
	}, [q, role, status, verifiedF, emailVerifiedF, page, sort]);
	...
	async function changeStatus(id: string, next: Status) {
		...
		const p = patchUser(id, { status: next });
	...
	return (
		<div className='px-4 max-w-7xl mx-auto py-6 min-h-[calc(100svh-5rem)] flex flex-col overflow-x-hidden'>
			...
			<div className='flex flex-wrap gap-2 mb-4'>
				...
				{verifiedF !== 'any' && (
					<Badge variant='outline' className='text-xs'>
						Verificado: {verifiedF === 'true' ? 'sí' : 'no'}
					</Badge>
				)}
				{emailVerifiedF !== 'any' && (
					<Badge variant='outline' className='text-xs'>
						Email verificado: {emailVerifiedF === 'true' ? 'sí' : 'no'}
					</Badge>
				)}
			</div>
			...
			<div className='rounded-md border overflow-x-auto w-full'>
				<Table className='min-w-[1200px]'>
					...
					<TableBody>
						...
						{isError && (
							<TableRow>
								<TableCell colSpan={8}>Error al cargar usuarios.</TableCell>
							</TableRow>
						)}
							...
							<TableRow key={u.id}>
								<TableCell className='whitespace-nowrap'>
									<StatusInlineControl
										...
										onChange={(next) => changeStatus(u.id, next)}
										loading={changingId === u.id}
									...
									<div className='flex items-center gap-2'>
										<Avatar>
											<AvatarImage
												src={
													u.avatar ??
													`https://api.dicebear.com/9.x/initials/svg?seed=${encodeURIComponent(u.name ?? u.email)}&backgroundType=gradient&radius=50&fontWeight=700`
												}
												alt={u.name ?? u.email}
											/>
											<AvatarFallback>
								...
								<TableCell>{u.dni ?? <span className='text-muted-foreground'>—</span>}</TableCell>
								<TableCell>
									<div className='flex items-center gap-2'>
										<span>{u.email}</span>
										{u.emailVerified ? (
				...
				<DialogContent>
					...
					{detailUser && (
						<div className='space-y-2 text-sm'>
							<div>
								<span className='text-muted-foreground'>Email:</span> {detailUser.email}
							</div>
							<div>
								<span className='text-muted-foreground'>Nombre:</span> {detailUser.name ?? '—'}
							</div>
							<div>
								<span className='text-muted-foreground'>Rol:</span> {detailUser.role}
							</div>
							<div>
								<span className='text-muted-foreground'>Verificado:</span> {detailUser.verifiedPublic ? 'Sí' : 'No'}
							</div>
							<div>
								<span className='text-muted-foreground'>Email verificado:</span>{' '}
								{detailUser.emailVerified ? 'Sí' : 'No'}
							</div>
							<div>
								<span className='text-muted-foreground'>Creado:</span> {new Date(detailUser.createdAt).toLocaleString()}
							</div>
						</div>
					)}
				</DialogContent>
			...
			<AlertDialog open={!!verifyDialog} onOpenChange={(o) => !o && setVerifyDialog(null)}>
				<AlertDialogContent>
					...
					<div className='space-y-2 text-sm text-muted-foreground'>
						<div>
							Marcar como verificado mostrará un distintivo público de verificación en el perfil y publicaciones
							relacionadas.
						</div>
						...
						<AlertDialogAction
							onClick={async () => {
								...
								await toast.promise(p, {
									...
									success: 'Verificado actualizado',
			...
			<AlertDialog open={!!deleteDialog} onOpenChange={(o) => !o && setDeleteDialog(null)}>
				<AlertDialogContent>
					...
					<div className='space-y-2 text-sm text-muted-foreground'>
						<div>
							Eliminar permanentemente borra el usuario y sus datos asociados (publicaciones, comentarios y actividad).
						</div>
			...
			<Dialog open={!!rejectDialog} onOpenChange={(o) => !o && setRejectDialog(null)}>
				<DialogContent>
					...
					<div className='space-y-3'>
						<Label htmlFor='reason-user'>Explicá brevemente el motivo</Label>
						<Textarea
							id='reason-user'
							value={rejectReason}
            ...
            <div>
              <Label>Ordenar por</Label>
						...
						<div>
							<Label>Verificado público</Label>
							<Select value={verifiedF} onValueChange={(v) => setVerifiedF(v as 'any' | 'true' | 'false')}>
								<SelectTrigger className='w-full'>
									<SelectValue placeholder='Verificado público' />
								</SelectTrigger>
								<SelectContent>
									<SelectGroup>
										<SelectItem value='any'>Todos</SelectItem>
						...
						<div>
							...
							<Select value={emailVerifiedF} onValueChange={(v) => setEmailVerifiedF(v as 'any' | 'true' | 'false')}>
								<SelectTrigger className='w-full'>
